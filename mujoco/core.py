...
def eval(env: gym.vector.Env, model: DecisionTransformer, rtg_target):
    ...
    state_dim = utils.extract_space_shape(env.observation_space, is_vector_env=True)
    act_dim = utils.extract_space_shape(env.action_space, is_vector_env=True)
...
def train(cfg, seed, log_dict, idx, logger, buffer_dir):
    ...
    utils.seed_everywhere(eval_env, seed)

    state_dim = utils.extract_space_shape(eval_env.observation_space, is_vector_env=True)
    action_dim = utils.extract_space_shape(eval_env.action_space, is_vector_env=True)
    ...
    best_reward = -np.inf
    utils.write_to_dict(local_log_dict, 'rtg_target', cfg.rtg_target)
    for timestep in range(1, cfg.train_steps + cfg.finetune_steps + 1):
        ...
        if timestep % cfg.eval_interval == 0:
            ...
            d4rl_score = utils.calculate_d4rl_normalized_score(env_name, eval_mean)
            utils.write_to_dict(local_log_dict, 'd4rl_score', d4rl_score)
            ...
            if eval_mean > best_reward:
                ...
                model.save_model(f'best_train_seed_{seed}' if timestep <= cfg.train_steps else f'best_finetune_seed_{seed}')
                logger.info(f'Seed: {seed}, Save best model at eval mean {best_reward:.4f} and step {timestep}')

        if timestep % cfg.plot_interval == 0:
            utils.sync_and_visualize(log_dict, local_log_dict, idx, timestep, f'{env_name} {buffer.dataset.title()}')

        if timestep == cfg.train_steps:
            model.save_model(f'final_train_seed_{seed}')
            model.load_model(f'best_train_seed_{seed}')

            perf_drop_curve = get_perf_drop_curve(eval_env, model, cfg.rtg_target, cfg.eval_drop_ps, seed)
            for drop_perf in perf_drop_curve:
                utils.write_to_dict(local_log_dict, 'perf_drop_train', drop_perf)
            if cfg.finetune_steps > 0 and model.drop_aware:
                ...
                best_reward = -np.inf # ensure to save best finetune model at least once
    
    if cfg.finetune_steps > 0 and model.drop_aware:
        model.save_model(f'final_finetune_seed_{seed}')
        model.load_model(f'best_finetune_seed_{seed}')

        perf_drop_curve = get_perf_drop_curve(eval_env, model, cfg.rtg_target, cfg.eval_drop_ps, seed)
        for drop_perf in perf_drop_curve:
            utils.write_to_dict(local_log_dict, 'perf_drop_finetune', drop_perf)
